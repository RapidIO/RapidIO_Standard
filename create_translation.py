...
"""
    ...
    Reads in two types of files:
    base_x.xtoz.z : Determine what sections are new in z.z
    translate_x.xtoz.z: Determine the translation between x.x and z.z
...
class RapidIOOutlineMerger(object):
    ...
    def __init__(self, outlines, translations):
        ...
        for outline in sorted(outlines):
            ...
            for i, line in enumerate(outline_lines[1:]):
                tokens = [token.strip() for token in line.split("',")]
                tokens = [re.sub("'", "", token) for token in tokens]
                try:
                    revision = tokens[0]
